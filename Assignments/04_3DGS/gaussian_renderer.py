...
class GaussianRenderer(nn.Module):
    ...
    def compute_projection(
        ...
        z = cam_points[..., 2]
        J_proj[..., 0, 0] = K[0, 0] / z
        J_proj[..., 0, 2] = -K[0, 0] * cam_points[..., 0] / z**2
        J_proj[..., 1, 1] = K[1, 1] / z
        J_proj[..., 1, 2] = -K[1, 1] * cam_points[..., 1] / z**2
    ...
    def compute_gaussian_values(
        ...
        # Compact Gaussian calculation
        inv_cov = torch.linalg.inv(covs2D)  # (N, 2, 2)
        dx = dx.unsqueeze(-1)  # (N, H, W, 2, 1)
        exponent = -0.5 * (dx.transpose(-2, -1) @ inv_cov.unsqueeze(1).unsqueeze(1) @ dx).squeeze()
        gaussian = torch.exp(exponent) / (2 * np.pi * torch.sqrt(torch.linalg.det(covs2D) + 1e-8).view(N, 1, 1))
    ...
    def forward(
        ...
        T = torch.cumprod(1.0 - weights + 1e-10, dim=0)  # (N, H, W)
        weights = weights * T  # (N, H, W)
